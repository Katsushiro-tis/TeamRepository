...
public class LoginCheckFilter implements Filter {
	...
	private boolean checkRequestURL(HttpServletRequest httpRequest) {
		...
		if (!URLCheck.checkURLForStaticFile(requestURL) && !requestURL.endsWith("/login")
				...
				&& !requestURL.endsWith("/user/regist/complete") && requestURL.indexOf("/basket/list") != -1
				&& !requestURL.endsWith("/order/list")) {
			System.out.println(requestURL + " : 非会員用実行対象だよ");
			// URLのリクエスト先がフィルタ実行対象である場合
			return true;
		} else {
			System.out.println(requestURL + " : 非会員用実行対象じゃないよ");
