...
//	FavoriteRepository favoriteRepository;
	...
	/*
	 * @RequestMapping(path = "/favorite/add", method = RequestMethod.GET) public
	 * String addFavoriteList(Model model, FavoriteForm form, HttpSession session) {
	 * 
	 * // 商品IDに該当する商品情報を取得 Item item =
	 * itemRepository.getById(Integer.valueOf(form.getId()));
	 * 
	 * // userIDに該当する商品情報を取得 UserBean sessionUser = (UserBean)
	 * session.getAttribute("user"); User user =
	 * userRepository.getById(sessionUser.getId());
	 * 
	 * // favoriteにセーブ Favorite favorite = new Favorite();
	 * 
	 * favorite.setItem(item); favorite.setUser(user);
	 * 
	 * List<Favorite> favoriteadd = favoriteRepository.findByItemAndUser(item,
	 * user); List<FavoriteBean> itemBeanList4 =
	 * BeanCopy.copyEntityToFavoriteBean(favoriteadd);
	 * 
	 * for (FavoriteBean f : itemBeanList4) {
	 * //すでに保存されているItem/Userと今回addしたItem/Userが同じならsaveせずにリダイレクト if(item.getName() ==
	 * f.getItem().getName() && user.getName() == f.getUser().getName()) { return
	 ...
	 * String showFavoriteList(Model model) { List<Favorite> favoriteitems =
	 * favoriteRepository.findAll();
	 * 
	 * List<FavoriteBean> itemBeanList3 =
	 * BeanCopy.copyEntityToFavoriteBean(favoriteitems);
	 * 
	 * model.addAttribute("items", itemBeanList3); return
	 * "/item/list/item_favorite"; }
	 * 
	 * @PostMapping("/favorite/delete") public String deleteFavoriteItem(HttpSession
	 * session, FavoriteForm favoriteForm) {
	 * favoriteRepository.deleteById(Integer.valueOf(favoriteForm.getId())); return
	 * "redirect:/favorite/list"; }
	 */
